...
RSpec.describe NotificationMailer, type: :mailer do
  ...
  describe '#under_review_task' do
    ...
    it 'has the correct body' do
      expect(CGI.unescapeHTML(email.body.to_s)).to include("#{user.display_name} marked task #{task.title} for project #{link_to project.title, project_url(project.id)} as finished.")
    end
  end
  ...
  describe '#comment' do
    ...
    it 'has the correct body' do
      expect(CGI.unescapeHTML(email.body.to_s)).to include("#{task_comment.user.display_name} added a comment to task ")
    end
  end
  ...
  describe '#revision_approved' do
    ...
    it 'has the correct body' do
      expect(CGI.unescapeHTML(email.body.to_s)).to include("#{leader.display_name} changed the approved version of the project #{link_to project.title, project_url(project.id)}.")
    end
  end
  ...
  describe '#task_started' do
    ...
    it 'has the correct body' do
      expect(CGI.unescapeHTML(email.body.to_s)).to include("#{user.display_name} started task #{task.title} of project #{link_to(task.project.title, project_url(task.project.id))}.")
    end
  end
  ...
  describe '#task_deleted' do
    ...
    it 'has the correct body' do
      expect(CGI.unescapeHTML(email.body.to_s)).to include("Task #{task.title} for project #{link_to task.project.title, project_url(task.project.id)} was deleted by #{admin.display_name}")
    end
  end
