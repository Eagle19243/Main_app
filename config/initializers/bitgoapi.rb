module Bitgo
	module V1
		...
		class Api
			...
			# }
			def simple_create_wallet(passphrase: passphrase, label: label, access_token: access_token )
			...
			def get_wallet(wallet_id: wallet_id,access_token: access_token )
				@session_token = access_token
			...
			def create_address(wallet_id: wallet_id, chain: chain, access_token: access_token)
				@session_token = access_token
				call :post, '/wallet/' + wallet_id + '/address/' + chain
			end

			def send_coins_to_address(wallet_id: wallet_id, address: address, amount: amount, wallet_passphrase: wallet_passphrase, min_confirmations: min_confirmations, fee: fee, access_token:access_token)
				@session_token = access_token
